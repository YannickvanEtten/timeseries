...
def plot_kalman_filter(df,a,v,F,K,P):
    ...
    axs[0, 0].plot(df['Year'][1:], a_upper[1:],color='orange')
...
def plot_kalman_smoother(df,alpha,V,r,N):
    ...
    axs[0, 0].plot(df['Year'], alpha_upper,color='orange')
...
def simulation(df,sigma_sq_eta,sigma_sq_eps,epsilon_hat,eta_hat,epsilon_plus,eta_plus):
    ...
    a_hat = np.zeros(number_obs)
    v_hat = np.zeros(number_obs)
    F_hat = np.zeros(number_obs)
    K_hat = np.zeros(number_obs)
    P_hat = np.zeros(number_obs)
    
    a_hat[0] = 0
    P_hat[0] = 10**7
    
    for i in range(number_obs):
        v_hat[i] = y_plus[i] - a_hat[i]
        F_hat[i] = P_hat[i] + sigma_sq_eps
        K_hat[i] = P_hat[i]/F_hat[i]
    
        if(i != number_obs - 1):
            a_hat[i + 1] = a_hat[i] + K_hat[i]*v_hat[i]
            P_hat[i + 1] = K_hat[i]*sigma_sq_eps + sigma_sq_eta
         
    r_hat = np.zeros(number_obs)
    N_hat = np.zeros(number_obs)
    alpha_hat = np.zeros(number_obs)
    V_hat = np.zeros(number_obs)
    
    r_hat[number_obs-1] = 0
    N_hat[number_obs-1] = 0
    
    for i in reversed(range(number_obs)):
        r_hat[i-1] = v_hat[i]/F_hat[i] + (1 - K_hat[i])*r_hat[i]
        alpha_hat[i] = a_hat[i] + P_hat[i]*r_hat[i-1]
        N_hat[i-1] = 1/F_hat[i] + (1 - K_hat[i])**2 * N_hat[i]
        V_hat[i] = P_hat[i] - ((P_hat[i]**2) * N_hat[i-1])
    
    u_hat = v_hat/F_hat - K_hat*r_hat
    epsilon_hat_plus = sigma_sq_eps*u_hat
    
    epsilon_tilde = epsilon_plus - epsilon_hat_plus + epsilon_hat
    
    alpha_tilde = df['Nile'] - epsilon_tilde
...
def main():
    ...
    mean_eps = 0
    mean_eta = 0
    epsilon_plus = mean_eps + sigma_sq_eps**(0.5) * np.random.normal(0, 1, number_obs)
    eta_plus = mean_eta + sigma_sq_eta**(0.5) * np.random.normal(0, 1, number_obs)
    ...
    #figure 1
    a,v,F,K,P = kalman_filter(df_nile,sigma_sq_eta,sigma_sq_eps)
    ...
    plot_simulation(df_nile,alpha_hat,alpha_plus,alpha_tilde,epsilon_hat,epsilon_tilde,eta_hat,eta_tilde)
